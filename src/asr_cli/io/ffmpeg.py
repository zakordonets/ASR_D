...
class FfmpegPreprocessor:
    ...
    def inspect(self, path: Path) -> dict:
        ...
        try:
            ...
            raise PreprocessingError(f'ffprobe failed for {path}: {exc.stderr.strip()}') from exc
    ...
    def prepare(self, path: Path, workspace: Path) -> PreparedMedia:
        ...
        try:
            ...
            raise PreprocessingError(f'ffmpeg failed for {path}: {exc.stderr.strip()}') from exc
        source_kind = (
            'video'
